...
!###     CALL MPI_BARRIER(MPI_COMM_WORLD, ierr) 
   ...
   !   IF (Rank_of_process==1) print*,'electron_to_add(k)%X,wil_4',electron_to_add(1707)%X
     ...
     IF (n_sub.EQ.N_subcycles .AND. i_freeze_ions==0) THEN            ! N_subcycles is odd
      ...
      !   CALL MPI_BARRIER(MPI_COMM_WORLD, ierr)
        CALL CONST_DENSITY_IONIZATION ! needs to be called after COLLECT_PARTICLE_BOUNDARY_HITS and before CLEAR_COULOMB_ARRAYS
        CALL CONST_TEMPERATURE ! self-contained, call anywhere
